...
function promisifyAll(obj) {
    _.functionsIn(obj).forEach(fn => {
        var f = promisify(obj[fn]).bind(obj);
        obj[fn + 'Async'] = f;
    })
    return obj;
}
...
function getSheetFromInfo(info, titleOrIndex=0) {
    ...
    var s = _.find(info.worksheets, sheet => sheet.title.toLowerCase() === titleOrIndex);
...
function findColumn(columns, value) {
    ...
    return _.find(columns, c => c.key === toFind);
}
...
function createKeyMap(keyCells,columns, keyField) {
    const col = findColumn(columns, keyField);
    return keyCells
        .filter(c => c.col === col.col && !_.isEmpty(c.value) )
        .map(c=>({
            value: c.value,
            row: c.row,
 ...
 */
async function merge({ creds, spreadsheetId, sheet = 0, data, headerRow=1, keyField, addMissing=true, removeRows=false }) {    
    const currentSheet = await getSheet(spreadsheetId, creds, sheet);
    ...
    let deleted = [];
    data.forEach(item => {
        const matchedKey = _.find(keyMap, { value: item[keyField] });
        if (!matchedKey) {
            missing.push(item)
            return;
        }
        const matchedCells = _.filter(cells, { row: matchedKey.row }); 
